...
function write_clusters_to_csv(dimensions::Int64, positive_size::Int64, negative_size::Int64)
  # Generate two clusters random data points (magic multiplicative factor of 32).
  positive_center = (rand(Float64, dimensions) .- 0.5) * 32
  negative_center = (rand(Float64, dimensions) .- 0.5) * 32

  # Generate clusters of points uniformly distributed around each center (magic radius of 1.0).
  positive_cluster = Cluster.create_uniform_cluster(positive_center, 1.0, positive_size)
  negative_cluster = Cluster.create_uniform_cluster(negative_center, 1.0, negative_size)

  CSV.write("positive_data.csv", DataFrame(positive_cluster), writeheader=false)
  CSV.write("negative_data.csv", DataFrame(negative_cluster), writeheader=false)

  return positive_center, negative_center
end

function read_clusters_from_csv()
  positive_data = Matrix(CSV.read("positive_data.csv", header=false, DataFrame))
  negative_data = Matrix(CSV.read("negative_data.csv", header=false, DataFrame))
